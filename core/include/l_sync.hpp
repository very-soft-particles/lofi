...
namespace lofi {
  ...
  public:
    ...
    atomic_counter operator=(count_t _count) {
      return count.store(_count);
    }
    ...
    count_t clear() {
      ...
      while(!count.compare_exchange_weak(current, 0)) {
      }
      return current;
    }
...
//  }
  ...
  private:
    ...
    public:
      ...
      void pop(node** first_ptr, node** last_ptr, u64 count) {
        size_t i = 0;
        *first_ptr = first;
        while(first) {
          i++;
    ...
    void push(size_t thread_id, node* new_node) {            // returns pointer to newly created node
      ...
      for(size_t i = thread_id; i < NumThreads + thread_id; i++) {
        ...
        if(thread_local_pools[index].grab()) {
          ...
          thread_local_pools[index].push(new_node);
